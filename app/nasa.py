...
def resolve_date(requested_date: str | None = None) -> Date:
    ...
    if requested_date is None: return today

    if re.fullmatch(r"\d{4}-\d{2}-\d{2}", requested_date) is None: raise Error(Code.INVALID_DATE_FORMAT) # check regex
    try: requested_date = Date.fromisoformat(requested_date) # convert to an actual date
    except ValueError: raise Error(Code.INVALID_DATE_FORMAT)
    if requested_date < Date(1995, 6, 16): raise Error(Code.DATE_TOO_EARLY) #  first day of apod
    elif requested_date > today: raise Error(Code.DATE_IN_FUTURE) # overshot
...
async def fetch_apod(
    ...
    match response.status_code:
        ...
        case 404: raise Error(Code.NOT_FOUND) # no apod found for that date (should never happen, can try to hit ellanan wrapper ig, prolly not, maybe nasa apod website scrapper)
